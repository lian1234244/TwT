...
- 更新体验修复：发现新版本时自动弹出更新窗口（不再只有右上角小图标），右上角更新图标只在真正有新版本时常驻显示。
- 修复更新假下载死路：移除演示用的模拟下载进度条，无更新时按钮显示"当前已是最新"；此前的"正式接入后将重启并安装新版"死提示已删除。
- 更新检测失败时静默 60 秒自动重试，运行期间每 30 分钟复查一次新版本。
